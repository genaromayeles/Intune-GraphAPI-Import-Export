...
######################################
# Export Mobile Threat Defense (MTD) Connectors #
######################################

try {
    $mtdConnectorsRequest = (Invoke-RestMethod -Headers $HeaderParams -Uri "https://graph.microsoft.com/v1.0/deviceManagement/mobileThreatDefenseConnectors" -Method Get)
    $mtdConnectors = $mtdConnectorsRequest.value

    foreach ($connector in $mtdConnectors) {
        $filePath = "$($logfolder)\MobileThreatDefenseConnector - $($connector.displayName).json"
        
        # Skip if file already exists
        if (Check-FileExistence -filePath $filePath) { continue }
        
        # Retrieve full details of the MTD connector
        $connectorDetails = (Invoke-RestMethod -Headers $HeaderParams -Uri "https://graph.microsoft.com/v1.0/deviceManagement/mobileThreatDefenseConnectors/$($connector.id)" -Method Get)
        
        # Export connector with all details if no duplicate exists
        $connectorDetails | ConvertTo-Json -Depth 10 | Out-File $filePath
        Write-Log "Exported Mobile Threat Defense Connector: $($connector.displayName)"
    }
}
catch {
    Write-Log "Error exporting Mobile Threat Defense Connectors: $($_.Exception.Message)"
}

Write-Host "Mobile Threat Defense Connectors successfully exported!" -ForegroundColor Yellow
