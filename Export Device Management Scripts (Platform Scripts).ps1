...
######################################
# Export Device Management Scripts (Platform Scripts) #
######################################
...
try {
    ...
    foreach ($script in $deviceManagementScripts) {
        $filePath = "$($logfolder)\DeviceManagementScript - $($script.displayName).json"
        
        # Skip if file already exists
        if (Check-FileExistence -filePath $filePath) { continue }
        
        # Retrieve full details of the device management script
        $scriptDetails = (Invoke-RestMethod -Headers $HeaderParams -Uri "https://graph.microsoft.com/beta/deviceManagement/deviceManagementScripts/$($script.id)" -Method Get)
        
        # Export script with all details if no duplicate exists
        $scriptDetails | ConvertTo-Json -Depth 10 | Out-File $filePath
        Write-Log "Exported Device Management Script: $($script.displayName)"
    }
}
catch {
    Write-Log "Error exporting Device Management Scripts: $($_.Exception.Message)"
}

Write-Host "Device Management Scripts successfully exported!" -ForegroundColor Yellow
