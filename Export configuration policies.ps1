...
try {
    $configurationPoliciesRequest = (Invoke-RestMethod -Headers $HeaderParams -Uri "https://graph.microsoft.com/v1.0/deviceManagement/deviceConfigurations" -Method Get)
    $configurationPolicies = $configurationPoliciesRequest.value

    if ($null -eq $configurationPolicies) {
        Write-Log "No configuration policies found."
    }
    else {
        foreach ($policy in $configurationPolicies) {
            $filePath = "$($logfolder)\ConfigurationPolicy - $($policy.displayName).json"
            
            # Skip if file already exists
            if (Check-FileExistence -filePath $filePath) { continue }
            
            # Retrieve full details of the configuration policy
            $policyDetails = (Invoke-RestMethod -Headers $HeaderParams -Uri "https://graph.microsoft.com/v1.0/deviceManagement/deviceConfigurations/$($policy.id)" -Method Get)
            
            # Export policy with all details if no duplicate exists
            $policyDetails | ConvertTo-Json -Depth 10 | Out-File $filePath
            Write-Log "Exported configuration policy: $($policy.displayName)"
        }
    }
}
catch {
    Write-Log "Error exporting configuration policies: $($_.Exception.Message)"
